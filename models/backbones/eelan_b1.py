...
class EELANBlock1(pytorch_lightning.LightningModule):
    ...
    def forward(self, x):
        ...
        x = self.stem(x)
        outputs["stem"] = x
        outputs["stem_exit"] = self.stem_exit(self.stem_exit1(self.stem_exit0(x)))
        x = self.block1(x)
        outputs["block1"] = x
        outputs["block1_exit"] = self.block1_exit(self.block1_exit1(self.block1_exit0(x)))
